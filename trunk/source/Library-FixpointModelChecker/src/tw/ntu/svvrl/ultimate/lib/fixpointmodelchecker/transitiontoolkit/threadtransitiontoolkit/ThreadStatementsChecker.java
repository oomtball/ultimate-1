package tw.ntu.svvrl.ultimate.lib.fixpointmodelchecker.transitiontoolkit.threadtransitiontoolkit;
...
import tw.ntu.svvrl.ultimate.lib.fixpointmodelchecker.transitiontoolkit.StatementsChecker;
		...
		for(int i = 0; i < mStatements.size(); i++) {
			...
			if(stmt instanceof AssumeStatement) {
				...
				final ThreadStatementsExecutor stmtsExecutor
					= new ThreadStatementsExecutor(stmt, mState, ThreadStatementsExecutor.ExecStrategy.check, mProgramStateExplorer);
